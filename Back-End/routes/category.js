...
router.get("/get", auth.authenticateToken, (req, res) => {
  ...
  var queryCount =
    "SELECT COUNT(*) as dataCount FROM category WHERE deleted='false' and (? IS NULL or name LIKE ?) ORDER BY id ASC";
  let dataCount = 0;
  connection.query(
    queryCount,
  ...
  var query =
    "SELECT * FROM category WHERE deleted='false' and (? IS NULL or name LIKE ?) order by id asc LIMIT ? OFFSET ?";
  connection.query(
    query,
    [valueSearch, ["%" + valueSearch + "%"], valueLimit, valueOffset],
    (err, results) => {
      if (!err) {
        return res.status(200).json({
          results: {
        ...
        if (results.affectedRows == 0) {
          return res.status(200).json({
            results: {
              responseCode: "404",
              message:
                "Danh mục không được tìm thấy hoặc danh mục đã được chuyển vào thùng rác.",
            },
          });
        } else {
          return res.status(200).json({
...
router.get("/trash", auth.authenticateToken, (req, res) => {
  ...
  var queryCount =
    "SELECT COUNT(*) as dataCount FROM category WHERE deleted='true' and (? IS NULL or name LIKE ?) ORDER BY id ASC";
  let dataCount = 0;
  connection.query(
    queryCount,
    [valueSearch, ["%" + valueSearch + "%"]],
  ...
  var query =
    "SELECT * FROM category WHERE deleted='true' and (? IS NULL or name LIKE ?) order by id asc LIMIT ? OFFSET ?";
  connection.query(
    query,
    [valueSearch, ["%" + valueSearch + "%"], valueLimit, valueOffset],
    (err, results) => {
...
router.get("/search-trash", auth.authenticateToken, (req, res) => {
  var queryCount =
    "select COUNT(*) as dataCount from category where deleted='true' and name LIKE ? order by id asc";
  let dataCount = 0;
  connection.query(
    queryCount,
  ...
  var query =
    "select * from category where deleted='true' and name LIKE ? order by id asc";
  connection.query(query, [["%" + req.query.name + "%"]], (err, results) => {
    if (!err) {
      return res.status(200).json({
        results: {
