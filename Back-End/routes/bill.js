...
router.post("/generateReport", auth.authenticateToken, (req, res) => {
  ...
  var query =
    "insert into bill (name, uuid, email, contactNumber, paymentMethod, total, productDetails, createdBy, createdAt,deleted) values(?,?,?,?,?,?,?,?,?,'false')";
  connection.query(
    query,
    [
      orderDetails.name,
...
router.get("/getBills", auth.authenticateToken, (req, res) => {
  ...
  var queryCount =
    "SELECT COUNT(*) as dataCount FROM bill WHERE deleted='false' and (? IS NULL or name LIKE ?) ORDER BY createdAt desc";
  let dataCount = 0;
  connection.query(
    queryCount,
  ...
  var query =
    "SELECT * FROM bill WHERE deleted='false' and (? IS NULL or name LIKE ?) order by createdAt desc LIMIT ? OFFSET ?";
  connection.query(
    query,
      ...
      if (results.affectedRows == 0) {
        return res.status(200).json({
          results: {
            responseCode: "404",
            message:
              "Hóa đơn không được tìm thấy hoặc hóa đơn đã được chuyển vào thùng rác.",
          },
        });
      } else {
        return res.status(200).json({
...
router.get("/trash", auth.authenticateToken, (req, res) => {
  ...
  var queryCount =
    "SELECT COUNT(*) as dataCount FROM bill WHERE deleted='true' and (? IS NULL or name LIKE ?) ORDER BY createdAt desc";
  let dataCount = 0;
  connection.query(
    queryCount,
    [valueSearch, ["%" + valueSearch + "%"]],
  ...
  var query =
    "SELECT * FROM bill WHERE deleted='true' and (? IS NULL or name LIKE ?) order by createdAt desc LIMIT ? OFFSET ?";
  connection.query(
    query,
    [valueSearch, ["%" + valueSearch + "%"], valueLimit, valueOffset],
    (err, results) => {
