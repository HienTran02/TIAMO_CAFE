      ...
      if (results.length <= 0 || results[0].password != user.password) {
        ...
        return res.status(200).json({
          results: {
            responseCode: "404",
            message:
              "Tài khoản của bạn đã dừng hoạt động hoặc chưa kích hoạt. Liên hệ quản lý để kích hoạt",
          },
        });
        ...
        return res.status(200).json({
          results: {
            responseCode: "401",
            message:
              "Tài khoản của bạn đã bị xóa. Liên hệ quản lý để khôi phục",
          },
        });
      ...
      if (results.length <= 0) {
        ...
        var mailOptions = {
          ...
          html:
            ...
            "<br><b>Password: </b>" +
            results[0].password +
            '<br><a href="http://localhost:4200">Nhấn vào đây để đăng nhập lại</a></p>',
        };
...
router.get("/get", auth.authenticateToken, checkRole.checkRole, (req, res) => {
  ...
  var queryCount =
    "SELECT COUNT(*) as dataCount FROM user WHERE deleted='false' and (? IS NULL or name LIKE ?)";
  connection.query(
    queryCount,
  ...
  var query =
    "select id, name, email, contactNumber, status from user where role = 'user' and deleted='false' and (? IS NULL or name LIKE ?) order by id asc LIMIT ? OFFSET ?";
  connection.query(
    query,
